...
function normalizeBlock(value: unknown): LandingPageDocument["sections"][number]["blocks"][number] {
  ...
  if (type === "button") {
    ...
    const normalizedAction = action.type === "form" && typeof action.formId === "string" && action.formId ? { type: "form" as const, formId: action.formId } : action.type === "product" && typeof action.productId === "string" && action.productId ? { type: "product" as const, productId: action.productId } : action.type === "buy-now" && typeof action.pageId === "string" && action.pageId ? { type: "buy-now" as const, pageId: action.pageId } : { type: "url" as const, url: typeof action.url === "string" && action.url ? action.url : "https://example.com" };
...
function normalizeSection(value: unknown, index: number): LandingPageDocument["sections"][number] {
  ...
  return { id: typeof section.id === "string" && section.id ? section.id : `section-${index + 1}`, type: typeof section.type === "string" && section.type ? section.type : "custom", visible: section.visible !== false, blocks: (Array.isArray(section.blocks) ? section.blocks : []).map(normalizeBlock), settings: isRecord(section.settings) ? section.settings : {} };
}
...
export function normalizeLandingPageDocument(input: unknown): LandingPageDocument {
  ...
  return { schemaVersion: 1, sections: (Array.isArray(document.sections) ? document.sections : []).map(normalizeSection), theme: { colors: { primary: typeof colors.primary === "string" ? colors.primary : "#0325D9", primaryText: typeof colors.primaryText === "string" ? colors.primaryText : "#FFFFFF", surface: typeof colors.surface === "string" ? colors.surface : "#FFFFFF", text: typeof colors.text === "string" ? colors.text : "#111111", muted: typeof colors.muted === "string" ? colors.muted : "#6B7280" }, typography: { fontFamily: typeof typography.fontFamily === "string" ? typography.fontFamily : "Inter, system-ui, sans-serif" }, buttons, spacing: Object.fromEntries(Object.entries(spacing).filter(([, value]) => typeof value === "number" && Number.isFinite(value))) as Record<string, number>, radius: Object.fromEntries(Object.entries(radius).filter(([, value]) => typeof value === "number" && Number.isFinite(value))) as Record<string, number> }, references, metadata: { ...(typeof metadata.templateKey === "string" ? { templateKey: metadata.templateKey } : {}), aiGenerated: metadata.aiGenerated === true, ...(typeof metadata.aiPromptVersion === "string" ? { aiPromptVersion: metadata.aiPromptVersion } : {}) } };
}
