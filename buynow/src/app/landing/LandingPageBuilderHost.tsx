...
function applyProvidedLinks(document: LandingPageDocument, inputs: BuildInputs) {
  const next = structuredClone(document);
  if (inputs.formUrl.trim()) {
    const buttons = next.sections.flatMap((section) => section.blocks).filter((block) => block.type === "button");
    if (buttons.length) {
      const first = buttons[0];
      if (first.type === "button") first.action = { type: "url", url: inputs.formUrl.trim() };
    }
  }
  if (inputs.vslUrl.trim()) {
    const hasVsl = next.sections.some((section) => section.type === "vsl");
    if (!hasVsl) {
      const heroIndex = next.sections.findIndex((section) => section.type === "hero");
      next.sections.splice(Math.max(0, heroIndex + 1), 0, {
        ...
        blocks: [
          ...
          { type: "button", label: "Watch the VSL", action: { type: "url", url: inputs.vslUrl.trim() } },
        ],
        settings: { videoUrl: inputs.vslUrl.trim() },
      });
    }
  }
  return next;
...
function AIBuildSheet({ onClose, onApply }: { onClose: () => void; onApply: (document: LandingPageDocument, provider: AIBuildResponse["provider"], inputs: BuildInputs) => void }) {
  ...
  async function generate() {
    ...
    try {
      ...
      const inputs = { goal, audience, offer, outcome, price, cta, vslUrl, formUrl, templateKey, assets };
      onApply(applyProvidedLinks(attachAssets(response.document, assets, fitness), inputs), response.provider, inputs);
...
export default function LandingPageBuilderHost() {
  ...
  async function openBuilder(detail: LandingPageCreatedDetail) {
    ...
    try {
      ...
      setPage(response.page); setDocument(initialDocument); setShowAI(true); setAiSummary(null); setEditorVersion((value) => value + 1);
