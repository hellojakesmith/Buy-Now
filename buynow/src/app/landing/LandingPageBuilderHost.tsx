...
import { ArrowRight, Check, ChevronLeft, FileText, ImagePlus, Monitor, Sparkles, Smartphone, Tablet, Upload, X, Zap } from "lucide-react";
...
function sectionLabel(type: string) {
  return type === "social-proof" ? "Social proof" : type === "cta" ? "Call to action" : type === "faq" ? "FAQ" : type.charAt(0).toUpperCase() + type.slice(1).replace(/-/g, " ");
}
...
function attachAssets(document: LandingPageDocument, assets: UploadedAsset[], fitness: boolean) {
  ...
  if (fitness && assets.length >= 2) {
    ...
    proof.blocks = [
      ...assets.slice(1, 4).map((asset) => ({ type: "image" as const, assetId: `uploaded-${asset.id}`, src: asset.url, alt: asset.alt })),
      { type: "testimonial" as const, quote: "Add the real client transformation story here.", author: "Client transformation" },
    ];
  }
  return next;
}
...
function AIBuildSheet({ onClose, onApply }: { onClose: () => void; onApply: (document: LandingPageDocument, provider: AIBuildResponse["provider"], inputs: BuildInputs) => void }) {
  ...
  const goalOptions = [
    ["fitness", "🏋️ Fitness Coach", "Get coaching applications"],
    ["leads", "🎯 Generate Leads", "Capture qualified prospects"],
    ["sell", "🛍️ Sell Something", "Drive purchases"],
    ["book", "📅 Book Calls", "Turn visitors into appointments"],
  ];
  ...
  async function generate() {
    ...
    const prompt = [
      ...
      vslUrl.trim() ? `VSL URL: ${vslUrl.trim()}. Include a VSL/video section and use this exact URL.` : "",
      formUrl.trim() ? `Application/lead form URL: ${formUrl.trim()}. Use this exact URL for the primary CTA.` : "",
      fitness ? "This is a fitness coaching funnel. Prefer hero, VSL, problem, benefits, before/after social proof, testimonials, how it works, application CTA, FAQ, and final CTA. Never invent transformation metrics or testimonials." : "Prioritize a clear conversion path, trust, benefits, objection handling, and one primary CTA.",
      assets.length ? `The user uploaded ${assets.length} real image asset(s). Use the uploaded assets in the generated page rather than inventing image URLs.` : "Use the template's curated starter imagery where an image is helpful.",
    ].filter(Boolean).join("\n");
    try {
      const response = await apiRequest<AIBuildResponse>("/ai/landing-page", { method: "POST", body: JSON.stringify({ prompt, templateKey }) });
      const document = attachAssets(response.document, assets, fitness);
      onApply(document, response.provider, { goal, audience, offer, outcome, price, cta, vslUrl, formUrl, templateKey, assets });
    } catch (err) {
  ...
  return <div className="fixed inset-0 z-[300] flex items-end bg-black/45 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Build landing page with AI">
    <div className="max-h-[94vh] w-full overflow-y-auto rounded-t-[30px] bg-white p-5 pb-[max(1.5rem,env(safe-area-inset-bottom))] shadow-2xl">
      <div className="mx-auto max-w-2xl">
        <div className="mb-5 flex items-start justify-between gap-4"><div><div className="inline-flex items-center gap-1.5 rounded-full bg-[#EEE9FF] px-3 py-1 text-[11px] font-black text-[#5B21B6]"><Sparkles size={13} /> AI BUILD</div><h2 className="mt-3 text-[25px] font-black leading-tight">Build your landing page for you.</h2><p className="mt-2 text-[13px] leading-5 text-[#6B7280]">Give Buy Now the goal, offer, audience, and photos you already have. We'll turn them into a conversion-focused mobile page.</p></div><button type="button" onClick={onClose} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#F7F8FC]" aria-label="Close"><X size={18} /></button></div>
...
export default function LandingPageBuilderHost() {
  ...
  function handleCreated(event: Event) { void openBuilder((event as CustomEvent<LandingPageCreatedDetail>).detail); }
  ...
  return <div className="fixed inset-0 z-[100] bg-white">
    <LandingPageBuilderStudio key={`${page._id}-${editorVersion}`} pageId={page._id} initialDocument={document} onBack={() => { setPage(null); setDocument(null); setPreviewDocument(null); setShowAI(false); setAiProvider(null); setAiSummary(null); }} onPreview={setPreviewDocument} />
    {showAI && <AIBuildSheet onClose={() => setShowAI(false)} onApply={(nextDocument, provider, inputs) => { setDocument(nextDocument); setAiProvider(provider); setAiSummary(inputs); setEditorVersion((value) => value + 1); setShowAI(false); }} />}
    {!showAI && <div className="fixed bottom-[88px] right-4 z-[160] flex items-center gap-2"><button type="button" onClick={() => setShowAI(true)} className="flex items-center gap-2 rounded-full bg-[#111111] px-4 py-3 text-[12px] font-black text-white shadow-xl"><Sparkles size={15} /> Build with AI</button></div>}
    {aiSummary && !showAI && <div className="fixed left-4 right-4 top-4 z-[150] mx-auto max-w-md rounded-2xl border border-[#E5E7EB] bg-white p-4 shadow-xl"><div className="flex items-start gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#E8F8F1] text-[#16B879]"><Check size={17} /></div><div className="min-w-0 flex-1"><div className="text-[13px] font-black">Your page is ready</div><div className="mt-1 text-[11px] leading-4 text-[#6B7280]">Built for {aiSummary.audience}. {aiSummary.assets.length ? `${aiSummary.assets.length} of your photos were added.` : "Starter imagery was used."}</div><div className="mt-2 flex flex-wrap gap-1.5">{summarySections.slice(0, 7).map((item) => <span key={item} className="rounded-full bg-[#F7F8FC] px-2 py-1 text-[9px] font-bold text-[#6B7280]">{item}</span>)}</div></div><button type="button" onClick={() => setAiSummary(null)} className="text-[#9CA3AF]" aria-label="Dismiss"><X size={15} /></button></div></div>}
